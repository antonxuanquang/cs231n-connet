...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        scores -= np.max(scores, axis = 1, keepdims = True) # for safety, this won't cause numeric problem
        ...
        # W2 and b2
        grads['W2'] = hidden_layer.T.dot(d_softmax)
        grads['b2'] = np.sum(d_softmax, axis = 0)
        
        # hidden layer
        d_hidden_layer = d_softmax.dot(W2.T)
        
        # derivative of ReLU activation function
        d_hidden_layer[hidden_layer <= 0] = 0
        
        # W1 and b1
        grads['W1'] = X.T.dot(d_hidden_layer)
